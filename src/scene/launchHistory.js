...
function _missionCardHtml(m) {
  ...
  return `<div class="lh-mission-card${hasVideo ? ' lh-mission-card-expandable' : ''}">` +
    ...
    `</div>` +
    `<span class="lh-mission-card-org" style="background:rgba(100,160,255,0.06);border:1px solid rgba(100,160,255,0.12);color:rgba(160,185,220,0.65)">${m.org}</span>` +
...
function _missionRowHtml(m) {
  ...
  const statusColor = m.status === 'success' ? '#4ade80' : m.status === 'failed' ? '#f87171' : 'rgba(212,164,74,0.7)';
...
function _openOrgDetail(orgName) {
  ...
  const barHtml = barData.map(([y, c]) => {
    ...
    return `<div class="lh-od-bar-row"><span class="lh-od-bar-year">${y}</span><div class="lh-od-bar-track"><div class="lh-od-bar-fill" style="width:${pct}%;background:rgba(100,160,255,0.25)"></div></div><span class="lh-od-bar-val">${c}</span></div>`;
  }).join('');

  let html = `<div class="lh-od-header" style="border-bottom-color:rgba(60,80,120,0.15)">` +
    `<button class="lh-od-back" id="lh-od-back">\u2190 BACK</button>` +
    `<div class="lh-od-title" style="color:#e2e8f0">${orgName}</div>` +
    `</div>`;
  ...
  html += `<div class="lh-od-stats">` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${o.launches}</div><div class="lh-od-stat-lbl">Launches</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${rate}%</div><div class="lh-od-stat-lbl">Success Rate</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${massTonnes}t</div><div class="lh-od-stat-lbl">Mass to Orbit</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${yearRange}</div><div class="lh-od-stat-lbl">Active</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${o.firsts.length}</div><div class="lh-od-stat-lbl">Firsts</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${peakYear[0]}</div><div class="lh-od-stat-lbl">Peak Year (${peakYear[1]})</div></div>` +
    `</div>`;
  ...
  if (moreRockets > 0) html += `<div class="lh-od-rocket" style="color:rgba(140,165,200,0.3);font-style:italic">+ ${moreRockets} more rocket types</div>`;
  html += `</div></div>`;
  ...
  destList.forEach(d => {
    const dc = 'rgba(100,160,255,0.25)';
...
const _ERA_DEFS = [
  { label: 'THE SPACE RACE', range: [1957, 1969], color: '#7aa2f7', icon: '\u2606', desc: 'From Sputnik to Apollo \u2014 the era that launched humanity into space' },
  { label: 'STATIONS & SHUTTLES', range: [1970, 1999], color: '#7dcfff', icon: '\u2302', desc: 'Space stations, the Shuttle program, and international cooperation' },
  { label: 'EXPLORATION ERA', range: [2000, 2014], color: '#bb9af7', icon: '\u269B', desc: 'Mars rovers, ISS expansion, and the dawn of commercial spaceflight' },
  { label: 'COMMERCIAL REVOLUTION', range: [2015, 2022], color: '#ff9e64', icon: '\u26A1', desc: 'Reusable rockets, mega-constellations, and record launch cadence' },
  { label: 'THE NEW FRONTIER', range: [2023, 2030], color: '#e0af68', icon: '\u2B50', desc: 'Starship, Artemis, lunar return, and the push to Mars' },
];
...
function _openEraDetail(eraLabel, data) {
  ...
  let html = `<div class="lh-od-header" style="border-bottom-color:rgba(60,80,120,0.15)">` +
    `<button class="lh-od-back" id="lh-od-back">\u2190 BACK</button>` +
    `<div class="lh-od-title" style="color:#e2e8f0">${era.icon} ${era.label}</div>` +
    `</div><div class="lh-od-body">`;

  html += `<div class="lh-od-stats">` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${eraMissions.length.toLocaleString()}</div><div class="lh-od-stat-lbl">Launches</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${Math.round(successes/eraMissions.length*100)}%</div><div class="lh-od-stat-lbl">Success Rate</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${(totalMass/1000).toFixed(0)}t</div><div class="lh-od-stat-lbl">Mass to Orbit</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${orgs.size}</div><div class="lh-od-stat-lbl">Organizations</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${totalFirsts}</div><div class="lh-od-stat-lbl">Firsts</div></div>` +
    `<div class="lh-od-stat"><div class="lh-od-stat-val" style="color:#e2e8f0">${era.range[0]}\u2013${era.range[1]}</div><div class="lh-od-stat-lbl">Years</div></div>` +
    `</div>`;
  ...
  if (notable.length > 0) {
    ...
    notable.slice(0, 20).forEach(m => {
      const year = m.date.slice(0, 4);
      const oc = _getOC(m.org);
      html += `<div class="lh-era-first-card${m.video ? ' lh-era-first-expandable' : ''}" style="border-top-color:rgba(100,160,255,0.2)">` +
        (m.video ? _ytBadge : '') +
        `<div class="lh-era-first-year" style="color:#e2e8f0">${year}</div>` +
        `<div class="lh-era-first-name">${m.name}</div>` +
        `<div class="lh-era-first-org" style="color:#e2e8f0">${m.org}</div>` +
...
function _initEarthViewer(){
  ...
  orbitDefs.forEach((od, i) => {
    ...
    labelDiv.style.cssText = 'position:absolute;font-family:Orbitron,sans-serif;font-size:7px;color:rgba(180,200,230,0.45);letter-spacing:1px;pointer-events:none;white-space:nowrap';
...
function _initMarsViewer() {
  ...
  mOrbitDefs.forEach((od, i) => {
    ...
    lbl.style.cssText = 'position:absolute;font-family:Orbitron,sans-serif;font-size:7px;color:rgba(180,200,230,0.45);letter-spacing:1px;pointer-events:none;white-space:nowrap';
...
function _initSolarSystemViewer() {
  ...
  planetDefs.forEach(pd => {
    ...
    // Label
    const lbl = document.createElement('div');
    lbl.style.cssText = 'position:absolute;font-family:Orbitron,sans-serif;font-size:6px;color:rgba(180,200,230,0.45);letter-spacing:1px;pointer-events:none;white-space:nowrap';
